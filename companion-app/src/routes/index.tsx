...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Meet AeroSniffer: a multi-boot ESP32-S3 desk gadget. Cyber-Pet, Network Auditor, and Flight Radar — in one tiny pixel-faced friend." },
      { property: "og:title", content: "AeroSniffer — Pixel Companion" },
      { property: "og:description", content: "Three devices in one. A living desk companion with a pixel soul." },
    ],
  }),
  component: Index,
});
...
const FACE_ORDER: FaceState[] = [
  "startup", "idle", "happy", "excited", "surprised",
  "thinking", "love", "alert", "sleepy", "sad",
];
    ...
    <main className="pixelscape scanlines crt-flicker min-h-screen">
        ...
        <div className="max-w-6xl mx-auto px-6 pt-10 pb-20 grid md:grid-cols-2 gap-10 items-center min-h-[78vh]">
          ...
          <div className="order-1 md:order-2">
            ...
            <p className="font-mono-pixel mt-4 text-[color:var(--as-neon)]/80 text-xl md:text-2xl max-w-md">
              Three devices, one pixel soul. A multi-boot ESP32-S3 friend that
              lives on your desk and remembers your face.
            </p>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a href="#modes" className="pixel-btn">CHOOSE A MODE</a>
              <a href="#faces" className="pixel-btn pixel-btn-ghost">VIEW FACES</a>
            </div>
          </div>
        </div>
      ...
      <section id="faces" className="py-20 px-6 border-t border-[color:var(--as-neon)]/15">
        ...
        <p className="max-w-2xl mx-auto text-center font-mono-pixel text-[color:var(--as-neon)]/70 text-lg mt-3">
          Ten states. Three groups. Click one — he'll wear it. Each face is a 240×240 .bmp drawn from a 16-bit RGB565 palette.
        </p>
      ...
      <section id="companion" className="py-20 px-6 border-t border-[color:var(--as-neon)]/15">
        ...
        <p className="max-w-2xl mx-auto text-center font-mono-pixel text-[color:var(--as-neon)]/70 text-lg mt-3">
          Four pixel terminals to drive AeroSniffer over USB-C / Web Serial. Plug him in, pick a console.
        </p>
...
function ModeCard({
  n, name, face, desc, tags, active, onClick,
}: {
  n: number; name: string; face: FaceState; desc: string; tags: string[];
  ...
  return (
    <button
      ...
      style={active ? { boxShadow: `0 0 0 2px ${FACE_META[face].color}, 0 0 30px ${FACE_META[face].color}55` } : undefined}
    >
    ...
    <div className="max-w-6xl mx-auto mt-10 pixel-card p-6">
      ...
      <div className="grid md:grid-cols-2 gap-6 items-center">
        <div className="relative aspect-square max-w-[320px] mx-auto">
          ...
          {[[20,30],[70,40],[55,75],[35,60],[80,65]].map(([x,y], i) => (
            <div key={i} className="absolute w-2 h-2 bg-[color:var(--as-neon)]"
    ...
    <div className="max-w-6xl mx-auto mt-10 pixel-card p-6">
      ...
      <table className="w-full font-mono-pixel text-lg">
        <thead className="text-[color:var(--as-neon)]/60 font-pixel text-[10px]">
          <tr><th className="text-left py-2">CALLSIGN</th><th className="text-left">ALT</th><th className="text-left">SPD</th><th className="text-left">HDG</th></tr>
        </thead>
        <tbody>
          {flights.map((f) => (
            <tr key={f[0]} className="border-t border-[color:var(--as-neon)]/15">
              <td className="py-2 text-[color:var(--as-yellow)]">{f[0]}</td>
              <td>{f[1]}</td><td>{f[2]}</td><td>{f[3]}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function ToolCard({
  icon, color, title, desc, btn, href, onClick,
}: {
  icon: string; color: string; title: string; desc: string;
  btn: string; href: string; onClick?: () => void;
}) {
  return (
    <div
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="font-pixel text-sm" style={{ color }}>{title}</div>
      </div>
